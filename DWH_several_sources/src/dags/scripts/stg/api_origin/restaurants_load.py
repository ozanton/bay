...
limit = 50
offset = 0
restaurants_base_url = "________________________________"
...
class ApiDestRepository:
    def insert_data(self, conn: Connection, row) -> None:
        with conn.cursor() as cur:
            cur.execute(
                """
                    INSERT INTO stg.api_restaurants (object_value, update_ts)
...
class RestaurantsLoader:
    WF_KEY = "api_restaurants_origin_to_stg_workflow"
    LAST_LOADED_ID_KEY = "last_loaded_time"


    def __init__(self, url: restaurants_base_url, ip: InputParams, pg_dest: PgConnect, log: Logger) -> None:

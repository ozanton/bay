...
class TimestampsOriginRepository:
    ...
    def list_timestamps(self, timestamps_threshold: datetime, limit: int) -> List[TimestampsObj]:
        with self._db.client().cursor(row_factory=class_row(TimestampsObj)) as cur:
            cur.execute(
                """
                    select date_trunc('second', (object_value::JSON->>'delivery_ts')::timestamp) as ts,
                           extract (year from (object_value::JSON->>'delivery_ts') :: date) as year,
                           extract (month from (object_value::JSON->>'delivery_ts') :: date) as month,
                           extract (day from (object_value::JSON->>'delivery_ts') :: date) as day,
                           (object_value::JSON->>'delivery_ts') :: date as date,
                           (object_value::JSON->>'delivery_ts') :: time as time
                    from stg.api_deliveries
                    WHERE date_trunc('second', (object_value::JSON->>'delivery_ts')::timestamp) > %(threshold)s 
                    ORDER BY id ASC 
                    LIMIT %(limit)s; 
                """, {
...
class DeliveryTimestampsLoader:
    WF_KEY = "delivery_timestamps_stg_to_dds_workflow"
    ...
    def load_delivery_timestamps(self):
